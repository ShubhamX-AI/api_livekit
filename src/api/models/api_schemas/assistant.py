...
class UpdateAssistant(BaseModel):
    ...
    assistant_stt_model: Optional[Literal["native", "sarvam", "cartesia", "deepgram", "elevenlabs"]] = Field(None, description="Change the user-transcription source ('sarvam', 'cartesia', 'deepgram', 'elevenlabs' or 'native'). Ignored in realtime mode. 'cartesia', 'deepgram', 'elevenlabs' and 'native' are mutually exclusive with the wrong mode: 'native' is rejected in cascade. Sending it without assistant_stt_config resets the config to provider defaults.")
