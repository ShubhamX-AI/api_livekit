...
from src.core.model_support.tool_schema import build_tool_schema
...
def _build_raw_schema(tool_doc: Tool, strict_schemas: bool = False) -> dict:
    ...
    The schema itself is built by `model_support.tool_schema.build_tool_schema`, shared with
    the API validator and the request replay script so all three agree byte for byte on what
    the runtime sends. This wrapper only unpacks the document and logs.

    Raising on an over-long name (as `ToolNameTooLong`) skips this one tool — the caller logs
    and keeps going — instead of letting OpenAI reject the whole request, which would take
    every other tool with it.
    """
